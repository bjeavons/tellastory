...
async function intro(sender, channel, command, message) {
    ...
    if (command.help) {
        response = "b: This is Storytime. If you have a game token text it to start a story or join an in-progress one. Text /intro to learn more. Text /end to leave or end a game. FYI msg&data rates may apply.";
    }
        ...
        await gameplayer.message("b: OK, let's tell a story, you and I! I'll text you the start of a story and you text back to add on. Get ready!", player_id);
    }
    else if (isToken) {
        ...
        if (!game) {
            ...
            response = "b: Welcome to Storytime! Once at least one more person joins you'll be collaboratively telling a story over SMS! Learn more about what's going to happen, text back /intro (with slash).";
        }
            ...
            response = "b: You've joined a pending game. The game creator will start it once they're ready! Text /intro (with slash) to learn what will happen next or text /help for general game help.";
            await gameplayer.message(relayedMessage, storytime.getCreator(game));
        }
        else {
            let player_id = await gameplayer.create(sender, channel, 'active_game'); /b: @todo make const
            game = await storytime.join(player_id, message.toLowerCase());
            var relayedMessage = "b: Someone joined the game! Now at " + storytime.getPlayerCount(game) + " total players.";
            response = "b: You've joined an ongoing game! Text /intro (with slash) to learn what this is about or text /help for general game help.";
            await gameplayer.message(relayedMessage, storytime.getCreator(game));
        }
    }
    else {
        console.log("No story in progress");
        response = "b: This is Storytime! If you have a game token text it to start a story or join an in-progress one. Text /intro to learn more.";
    }
...
async function demo(player, command, message) {
    ...
    if (command.intro) {
        return "b: Storytime is a storytelling game over SMS. You and your friends tell a story, one or two words at a time, by text messages to this number. When someone texts the next part of the story the whole story gets sent to another player to add on to! Text /help for more info.";
    }
    ...
    if (command.stop) {
        ...
        await gameplayer.deactivate(player.key);
        return "b: I know, I'm not a great storyteller :b: Get some friends together with a game token and start telling a story! ";
    }
...
async function pregame(player, command, message) {
    ...
    if (command.help) {
        ...
        response = "b: Hi there, thanks for your message, I'm waiting for the game creator to let me get the story telling game started. Hang tight! Text /help for help.";
    }
...
async function gameplay(player, command, message) {
    ...
    if (command.stop && storytime.isCreator(game, player.key)) {
        ...
        response = "b: This is a SMS-based storytelling game. You and your friends tell a story, one or two words at a time, by text messages to this number. When someone texts the next part of the story the whole story gets sent to another player to add on to.";
    }
